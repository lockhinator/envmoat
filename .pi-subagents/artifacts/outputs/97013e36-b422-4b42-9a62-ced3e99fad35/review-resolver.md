...
## Note — `Resolve` does not resolve symlinks in starting directory

**Severity:** note (by design — Phase 4 deferral)
**File:** `resolver.go:36`
**Description:** `filepath.Clean(dir)` does not resolve symlinks. If the starting directory is reached via a symlink, `Resolve` walks up from the symlink path, not the canonical path. Verified empirically: `filepath.Clean("/tmp/link")` returns `/tmp/link`, not the target. This is consistent with the Phase 4 deferral noted in the spec ("no symlink following yet").
**Recommended fix:** None needed until Phase 4. At that point, consider `filepath.EvalSymlinks(dir)` before the walk.

---

## Note — `FindWalkRoot` does not expand `~` or enforce absolute path

**Severity:** bug (medium severity — usability)
**File:** `resolver.go:95-100`
**Description:** `filepath.Clean("~/projects")` returns `~/projects` (no tilde expansion). If `ENVMOAT_WALK_ROOT=~/projects`, the walk boundary comparison `dir == walkRoot` will never match because `dir` is absolute (from `filepath.Abs` or `os.Getwd`) but `walkRoot` is the literal `~/projects`. Verified empirically: `filepath.Abs("~/projects")` creates a literal `~/projects` subdirectory under the current dir. This means `ENVMOAT_WALK_ROOT` only works with absolute paths, and there is no validation or error message if a relative path is provided.
**Recommended fix:** Call `filepath.Abs(root)` after `filepath.Clean`, or document that `ENVMOAT_WALK_ROOT` must be an absolute path. Adding a validation with a clear error message would be best.

---

## Note — `Resolve` is not yet imported by any consumer

**Severity:** note
**File:** `internal/resolver/resolver.go`
**Description:** `grep` for `resolver.Resolve`, `resolver.ParseMarker`, `resolver.ResolveFromPWD`, or `resolver.FindWalkRoot` across all `.go` files returns zero matches. The resolver package is standalone with no integration point yet. The `cmd/root.go` commands (`load`, `init`, `set`, etc.) all print "not implemented yet". This is expected for Phase 4 work-in-progress.
**Recommended fix:** None — this is the expected state. Integration will happen in subsequent phases.

---

## Note — `go vet` and race detector pass cleanly

**Severity:** —
**File:** `internal/resolver/`
**Description:** `go vet ./internal/resolver/` produces no output (clean). `go test -race -parallel=4 -count=1 ./internal/resolver/` passes with no data races detected. The environment variable mutation tests (which set/unset `ENVMOAT_WALK_ROOT`) do not cause races because `go test` runs them sequentially by default, and the `-parallel=4` run also passed.
...
```acceptance-report
{
  ...
  "commandsRun": [
    ...
    {
      "command": "go vet ./internal/resolver/",
      "result": "passed",
      "summary": "no issues found"
    },
    {
      "command": "go test -race ./internal/resolver/ -count=1",
      "result": "passed",
      "summary": "no data races detected, 1.399s"
    },
    {
      "command": "go test -race -parallel=4 ./internal/resolver/ -count=1",
      "result": "passed",
      "summary": "no races under parallel execution, 1.254s"
    }
  ],
  "validationOutput": [
    "All 18 resolver tests pass (TestParseMarkerEmpty through TestMarkerContentValues)",
    "go vet ./internal/resolver/ — clean, no issues",
    "go test -race -parallel=4 — no data races detected",
    ...
    "Resolver package not yet imported by any consumer (expected for Phase 4 WIP)",
    "Store package uses .envmoat consistently (store.go:29 StoreDirName = \".envmoat\")",
    "cmd/root.go references .envmoat marker correctly in command descriptions"
  ],
  "residualRisks": [
    ...
    "FindWalkRoot does not enforce absolute path or expand ~ for ENVMOAT_WALK_ROOT",
    "Resolver package has no consumers yet — integration testing deferred to later phases"
  ],
  "noStagedFiles": true,
  ...
  "reviewFindings": [
    ...
    "bug (medium): resolver.go:95 - FindWalkRoot does not expand ~ or enforce absolute path for ENVMOAT_WALK_ROOT",
    ...
    "note: resolver.go:36 - Resolve does not follow symlinks (by design, Phase 4 deferral)",
    "note: resolver package has no consumers yet — integration deferred to later phases"
  ],
  "manualNotes": "Implementation is correct and well-structured. All 7 check items from the task verified. Additional deep-dive confirmed: go vet clean, race detector clean under parallel execution, no symlink resolution (by design for Phase 4), resolver not yet consumed by any cmd/ code (expected WIP state). The 6 findings are all non-blocking. ENVMOAT_WALK_ROOT with relative paths or ~ expansion is the most impactful gap — users setting it to ~/projects would get silent misbehavior."
}
```
